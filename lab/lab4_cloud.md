...
- [ ] Відкрийте в браузері сторінку реєстрації  IBM Cloud [https://cloud.ibm.com/registration](https://cloud.ibm.com/registration)        

- [ ] Введіть адресу пошти в поле Email і натисніть Enter. У випадку, якщо на вказану адресу немає зареєстрованого користувача, активуються інші поля для вводу 
- [ ] Після вводу усіх полів натисніть «Create Account».   

#### 1.2. Підтвердження аккаунту

- [ ] На вказану пошту повинно прийти повідомлення з пропозицією підтвердження . 

- [ ] Для підтвердження аккаунту натисніть «Confirm Account». Після вдалого підтвердження, повинно з’явитися вікно підтвердження  з якого можна перейти на сторінку входу натиснувши “Log In”. 
...
Інтерфейс консолі постійно змінюється. На момент створення даної лабораторної роботи він мав вигляд, як на рис.4. 1.

![](3_1media/6.png) 

рис.4.1. Інтерфейс консолі

- [ ] Огляньте головне меню, яке знаходиться вгорі сторінки. Коли ви входите в систему IBM Cloud, першою сторінкою, яку можна переглянути, є інформаційна панель, на якій відображаються віджети, що узагальнюють інформацію про стан вашого облікового запису. Далі можна керувати своїми ресурсами, перейшовши на піктограму Меню навігації -> Resource List. (рис.4.2)
...
- [ ] Перейдіть до каталогу. За замовченням там відобразяться тільки ті ресурси, які можна використовувати (добавляти) для Вашого тарифного плану, тобто lite (рис.4.3). 
- [ ] Використовуючи кнопку «filter» можна змінювати налаштування фільтру. 

![](3_1media/8.png) 

рис.4.3. Перегляд каталогу

#### 1.5. Налаштування аккаунту

- [ ] Перейдіть в меню Manage ->Account->Account Settings для перегляду налаштувань свого аккаунту (рис.4.4).  

![](3_1media/9.png) 

рис.4.9. Налаштування аккаунту
...
- [ ] Перейдіть в каталог IBM Cloud і виберіть пункт **Starter Kit**. Серед показаних пропозицій виберіть Node-RED Starter (рис.4.5).  

- [ ] Перейдіть на вкладку Software(рис.4.5), у “Offering type” виділіть опцію «Starter kits» (рис.4.5) , у переліку доступних сервісів виберіть «Node-RED App» (рис.4.5)   ![](3_1media/10.png)

рис.4.5. Створення стартового набору Node-RED starter

- [ ] У наступному вікні натисніть Create app (рис.4.6).

![](3_1media/10a.png) 

рис.4.6. Створення app 

- [ ] Заповніть сторінку аналогічно як на рис.4.7, вкажіть AppName (1) та Region=London(2).  Зверніть увагу на те, що AppName має бути унікальним, тому для даної лабораторної роботи це поле повинно включати ваше прізвище (наприклад Ivanenko) і ім’я (наприклад Ivan) англійською мовою без пробілів за наступним шаблоном: 
...
- [ ] Після заповнення натисніть “Create” (рис.4.7.поз.3)

![](3_1media/11.png)  

рис.4.7. Заповнення полів при створенні
...
- [ ] Для розгортання через сервіси керування життєвим циклом DevOPS необхідно налаштувати «Configure Continuous Delivery”, натиснути кнопку «Deploy Your App» (рис.4.8) 

 ![](3_1media/11a.png)

рис.4.8. Розгортання застосунку

- [ ] Повинно відкритися вікно конфігурування IBM Cloud Foundry (рис.4.9). У ньому виберіть регіон = London, кількість пам’яті = 256, і згенеруйте «IBM Cloud API key» кнопкою «New». Після цього натисніть на кнопку «Create» в правому верхньому кутку сторінки.   

![](3_1media/12.png) 

рис.4.9. Вікно конфігурування IBM Cloud Foundry 

- [ ] Перейдіть на [перелік Ваших ресурсів](https://cloud.ibm.com/resources) . 
- [ ] Почекайте кілька хвилин (десь 3-5), періодично оновлюйте сторінку , поки створиться і запуститься Ваш Cloud Foundry apps (рис.4.10), тобто перейде в стан “Started”.  

![](3_1media/13.png) 

рис.4.10. Вікно стану ресурсів 
...
- [ ] Використовуючи ліву бічну панель передивіться усі закладки ресурсу Cloudant (рис.4. 11). Зокрема подивіться які сервіси підключені до нього в закладці Connections. 
- [ ] В звіті поясніть що це за підключення.  

![](3_1media/14.png)  

рис.4.11. Головне вікно налаштування Cloudant 

#### 2.4. Перегляд стану та керування виконанням сервісу Node-RED  

- [ ] Використовуючи ліву бічну панель передивіться усі закладки ресурсу Node-RED (рис.4.12). 

Зокрема на закладці «Overview» можна подивитися стан сервісу. Наприклад «Stopped» вказує на те, що сервіс зупинено. Слід звернути увагу, що при невикористанні сервісу протягом певного періоду він автоматично зупиняється, про що буде повідомлено листом на пошту, яка використовувалась при реєстрації. Запуск, зупинка та перезапуск сервісу відбувається через праве меню керування сервісом (див. рис.4.12). 

- [ ] Якщо сервіс не виконується – запустіть його.

![](3_1media/15.png) 

рис.4.12. Запуск сервісу Node-RED

- [ ] Перейдіть на закладку бічної панелі Runtime. Передивіться зміст усіх можливостей: Memory and Instance, Environment variables та SSH. Зверніть увагу, що SSH дає можливість керувати контейнером Node.js (рис.4. 13).  

![](3_1media/16.png) 

рис.4.13. Вікно консолі SSH
...
- [ ] На сторінці привітання рис.4.14, натисніть “Next”.  
- [ ] Вкажіть ім’я та пароль користувача, які зможуть редагувати застосунки в Node-RED (рис.4.14) також обов’язково виставить опцію «Allow anyone to view the editor» і натисніть “Next”. 

![](3_1media/17.png) 

рис.4.14. Перший запуск Node-RED в хмарі

- [ ] Використовуючи кнопку “Next” перейдіть до останньої вкладки і натисніть “Finish”.  На стартовому вікні Node-RED натисніть кнопку “Go to your Node-RED flow editor”.

#### 3.2. Інсталяція Node-RED  

- [ ] Через зображення користувача зайдіть під своїм користувачем і паролем, які Ви налаштували до цього (4.15.). 

![](3_1media/18a.png) 

рис.4.15. Кнопка реєстрації користувача 
...
- [ ] Створіть просту програму, яка виводить текст отриманий з вузла Inject (рис.4.16). 
...
рис.4.16. Перехід до UI

#### 3.4. Передача даних з локального Node-RED на віддалений з використанням WEB-socket  

- [ ] Модифікуйте програму в хмарному Node-RED, як це показано на рис.4.17.

![](3_1media/20.png) 

рис.4.17. Використання WEB-socket серверного вузлу

- [ ] Запустіть Node-RED на локальному ПК. Створіть новий потік з назвою «Laba4_1», всі інші потоки деактивуйте. 
- [ ] Створіть програму, яка показана на рис.4. 21. При цьому першу частину шляху (яка замальована на рис.4.18) скопіюйте з Вашого хмарного UI. 

![](3_1media/21.png) 

рис.4.18. Використання WEB-socket клієнтського вузлу
...
- [ ] Модифікуйте програму в локальному Node-RED, як це показано на рис.4.19. Ця програма імітує змінні **ramp** та **sin** кожні 5 секунд. Перевірте її роботу.

 ![](3_1media/21a.png)

рис.4.19.  Програма імітації змінних
...
рис.4.20. Вікно Launch Cloudant Dashboard

#### 4.2. Вікна налаштування та адміністрування Cloudant    

- [ ] Після запуску вікна адміністрування Cloudant, автоматично відкриється вкладка Databases (рис.4.21). На цій вкладці видно всі бази даних (документи)  Cloudant. Після першого запуску Cloudant в переліку буде тільки база даних nodered.  

![](3_1media/23.png) 

рис.4.21. Вкладка Databases
...
рис.4.23. Запис в БД з хмарного Node-RED

- [ ] Зробіть розгортання проекту і запустіть на виконання. 
- [ ] Перевірте чи пишуться дані в БД. Запустіть Cloudant Dashboard. В переліку БД зайдіть в базу даних “db1”, натисніть її для перегляду. У списку документів повинні з’явитися записи, як на рис.4.24. 

![](3_1media/26.png) 

рис.4.24. Список документів

#### 4.5. Перегляд БД з Cloudant Dashboard    

- [ ] У списку документів виберіть табличне представлення (кнопка Table). Як видно дані відображаються у вигляді таблиці, я якій є поля _id, ramp та sin (рис.4.25).  

![](3_1media/27.png) 

рис.4.25. Перегляд БД з Cloudant Dashboard  
...
рис.4.26. Структура документу.

#### 4.6. Читання записів даних БД Cloudant з Node-RED    

- [ ] Вставте в хмарному Node-RED вузол `Cloudant in` і модифікуйте програму, як це показані на рис.4.27.  
...
- [ ] Зробіть розгортання проекту і перевірте його роботу. У вікні відлагодження після запуску Inject повинен сформуватися масив документів за весь час роботи. 
 ...
 рис.4.27. Читання записів даних БД Cloudant з Node-RED
...
рис.4.28. Код для добавлення в документи відмітки часу    
...
- [ ] Відкрийте Query і введіть запит, як на рис.4.29. і натисніть “RUN QUERY”. В результаті повинен буде виданий результат запиту.  

![](3_1media/31.png) 

рис.4.29.  Формування та перевірка користувацьких запитів find в Cloudant Dashboard
...
- [ ] На сторінці БД db1 створіть новий Search index, як це показано на рис.4.30. 
- [ ] Натисніть “Search Document and Build Index”.

![](3_1media/32.png)   

рис.4.30. Створення пошукових індексів в Cloudant

#### 4.10. Формування користувацьких запитів  в Node-RED    

- [ ] Модифікуйте програму в в хмарному Node-RED, як це показані на рис.4. 31. Зверніть увагу на пробіли! 
...
рис.4.31. Формування користувацьких запитів  в Node-RED
...
- [ ] Модифікуйте програму, як це показано на рис.4.32, нижче наведений лістинг функції “->Trend”, яку можне скопіювати для пришвидшення розробки. 
...
рис.4.31. Перегляд відібраних даних в Node-RED за допомогою трендів
